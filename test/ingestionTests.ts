...
import { DataSource, Document } from '../src/types/enterprise';
...
export async function runIngestionTests(testEnv: TestEnvironment): Promise<IngestionTestResult[]> {
  const results: IngestionTestResult[] = [];
...
async function testPdfIngestion(testEnv: TestEnvironment): Promise<IngestionTestResult> {
  try {
...
async function testJsonIngestion(testEnv: TestEnvironment): Promise<IngestionTestResult> {
  try {
...
async function testApiIngestion(testEnv: TestEnvironment): Promise<IngestionTestResult> {
  try {
...
async function testBatchIngestion(testEnv: TestEnvironment): Promise<IngestionTestResult> {
  try {
    console.log('  - Testing batch ingestion...');
    const startTime = Date.now();
    
    // Create batch data source
    const batchSource: DataSource = {
...
async function testOcrIngestion(testEnv: TestEnvironment): Promise<IngestionTestResult> {
  try {
    console.log('  - Testing OCR ingestion...');
    const startTime = Date.now();
    
    // Create OCR-enabled data source
    const ocrSource: DataSource = {
      id: 'test-ocr-source',
      tenantId: testEnv.testTenant.id,
